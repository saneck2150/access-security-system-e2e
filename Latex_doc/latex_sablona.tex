...
Implementácia podporuje aj konfigurovateľný režim \texttt{aad\_mode: "none"}, v ktorom sa AAD nepoužíva a hlavička nie je kryptograficky chránená. Tento režim existuje výhradne pre experimentálne porovnanie a umožňuje v analytickej časti demonštrovať presný dopad absencie AAD väzby.
...
Implementácia podporuje aj režim \texttt{key\_derivation\_mode: "direct"}, v ktorom sa HKDF nepoužíva a master kľúč slúži priamo ako AEAD kľúč pre všetky čítačky. Tento režim je určený iba pre experimentálne porovnanie, kde slúži na demonštráciu dôsledkov absencie per-reader izolácie.
...
Práve toto vzájomné prepojenie je jedným z hlavných predmetov analytickej časti. Experimenty s konfiguračnými profilmi budú ukazovať, čo sa stane, keď sa jednotlivé mechanizmy modifikujú: demonštrujú nie iba to, že zabezpečená verzia je \uv{lepšia}, ale aj to, ktorá konkrétna vrstva chráni proti ktorému konkrétnemu útoku.
...
\begin{itemize}
\item \textbf{AddressSanitizer (ASan)}: detekuje prístupy mimo hraníc bufferov (buffer overflow, underflow), použitie po uvoľnení (use-after-free), dvojité uvoľnenie (double-free) a ďalšie chyby práce s pamäťou za behu programu.
...
Testovacia sada obsahuje 44 testovacích prípadov (makrá \texttt{TEST()} frameworku Google Test) rozdelených do 7 testovacích binárnych súborov:
