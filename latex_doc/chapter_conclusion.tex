...
\midrule
T1 & Deterministický nonce bez runtime verifikácie (R1) zlepšuje kvalitu generovania (nezávislosť od RNG), ale neposkytuje pridanú \emph{detekčnú schopnosť} oproti náhodnému nonce (R0). Výsledky S3a/S3b sú identické: 100\% úspešnosť útoku pre R0 aj R1. \\
T2 & \texttt{ProtocolAnomalyDetector} (profily R2) eliminuje všetky testované útoky na 0\% úspešnosť. V scenároch S3 a S4 je jediným mechanizmom, ktorý útok zastaví. \\
...
T6 & Overhead detektora (5--10\% pokles priepustnosti, ${\sim}3$--$5\,\mu$s na frame) je rádovo nepodstatný pre reálnu prevádzku ($<$10 udalostí/s). \\
\bottomrule
\end{tabular}
\end{table}
...
Výsledky sú \textbf{plne deterministické} (nulová smerodajná odchýlka bezpečnostných metrík cez 5 behov), čo potvrdzuje, že pozorované rozdiely nie sú štatistickým artefaktom, ale dôsledkom architektonických vlastností systému.
...
C3 & Implementácia prototypu & Kapitola 16: 9 C++ modulov, ESP32 firmware, SQLite úložisko \\
C4 & Verifikácia kvality kódu & Kapitola 17: clang-tidy, sanitizers, 44 testov \\
C5 & Experimentálne hodnotenie & Kapitola 18: 6 profilov $\times$ 6 scenárov $\times$ 5 behov \\
\bottomrule
\end{tabular}
\end{table}
...
Tento diferencovaný pohľad --- nie \uv{zapni všetko}, ale \uv{pochop, čo chráni proti čomu} --- je hlavným záverom experimentálnej časti a, podľa mojej mienky, najdôležitejším prínosom tejto práce pre prax návrhu bezpečných systémov kontroly prístupu.
